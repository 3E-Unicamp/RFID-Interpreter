...
---------------------------------
Esta biblioteca foi obtida do seguinte link: https://github.com/abobija/esp-idf-rc522

O intuito de comentar ela, é para explicar de uma forma mais facilmente compreensível o seu funcionamento,
para que aqueles sem conhecimento prévio sobre o ESP-IDF possam de fato utilizar ela de forma assertiva.
*/

/*
Nesta primeira parte realizamos os includes relevantes para o funcionamento da biblioteca.
Como é convenção na programação em ESP, iremos utilizar freeRTOS:
freeRTOS é um sistema operacional bem simples que estaremos acrescentando no ESP32.
O que ele essencialmente faz, é separar a execução do nosso código em vários "threads" ou blocos
de código. Cada um deles será executado por um tempo configurável pelo programador. 

No caso de <esp_log.h>, ele será útil no momento de registrar erros durante a execução de alguma função.
Irei descrever mais o seu funcionamento ao longo do código.
*/
...
/*
A nossa primeira surpresa é o uso de um TAG, ele será essencialmente uma identificação que nós teremos
no momento de estar realizando o registro dos nossos erros.
Caso o TAG no nosso log for rc522, como apresentado aqui, sabemos que o erro especificado aconteceu
nesta biblioteca.
*/
...
/*
O struct utilizado neste código poderia ser visto como sendo um pseudo-registrador do ESP32, onde
iremos armazenar os dados relativos a bastantes coisas que vão ir acontecendo ao longo da utilização
desta biblioteca.

Note-se que temos vários tipos de  varíaveis "únicos" em C/C++. Eles surgem por conta do
"typedef" que existe na linguagem C, o que permite a gente criar os nossos próprios tipos de dados.
Os tipos de dados apresentados a continuação foram definidos e criados nas bibliotecas que estamos
incluindo neste código. Elas são as seguintes:
- rc522_config_t :
- TaskHandle_t :
...
/*
Aqui notamos diretamente o uso de um novo tipo de dado conhecido como o esp_err_t. Ele é usado
especificamente para a deteccão de erros no ESP32, e pode ser visto como um [signed int] com
propriedades especiais. Essencialmente:
- A depender do valor que seja retornado para toda função definida com esp_err_t, nós iremos ter um 
  código e uma mensagem de erro associado.
  Caso o valor retornado for ESP_OK, sabemos que não aconteceu erro.

Este tipo de variável é bastante útil e serve como uma ferramenta boa para realizar leitura de dados
mediante varredura; enquanto o erro estiver acontecendo, continuar checando o registrador X.
*/
